#!/usr/bin/env python3
""" ╔══════════════════════════════════════════════════════════════════════╗
    ║ ⚽ PREMIER LEAGUE PREDICTOR PRO v4.0 (HYBRID EDITION) ⚽             ║
    ║ ║
    ║ ✅ V4.0: Deep Stats Integration (Shots, Corners, Discipline from CSV)║
    ║ ✅ V4.0: ML features expanded to 68 features.                        ║
    ║ ✅ V4.0: CRITICAL BUG FIX - Scikit-Learn predict_proba classes_ sync ║
    ║ ✅ V4.0: Dixon-Coles Matrix upgraded to mg=10 (handles huge scores)  ║
    ║ ✅ FIXED: All previous v3.3 critical ML & data pipeline bugs         ║
    ╚══════════════════════════════════════════════════════════════════════╝ """
import requests
import json
import math
...
# ══════════════════════════════════════════════════════════════
def load_csv_history(csv_path: str = "E0 (1).csv") -> List[dict]:
    """Read deep stats from Football-Data.co.uk CSV"""
    if not Path(csv_path).exists():
        print(f"⚠️ Warning: Deep Stats CSV not found at {csv_path}")
        return []
...
class DixonColes:
    ...
    @staticmethod
    def predict(hxg, axg, rho=-0.13, mg=10):  # V4.0 FIX: mg=10
        hw = dr = aw = 0.0
        for i in range(mg):
            for j in range(mg):
    ...
    @staticmethod
    def matrix(hxg, axg, rho=-0.13, mg=10):   # V4.0 FIX: mg=10
        return {
...
# ══════════════════════════════════════════════════════════════
# ML v4.0 – DEEP STATS + CLASSES_ BUGFIX
# ══════════════════════════════════════════════════════════════
class MLPred:
    N_FEATURES = 68  # V4.0: Expanded from 59 to 68
    ...
    def train(self, data: DataProc, fixes: List[dict] = None, force_retrain: bool = False) -> bool:
        ...
        for idx, f in enumerate(sf):
            ...
            fake = {
                ...
                'utcDate': f.get('date', ''),
                'stats': f.get('stats', {}) # Pass deep stats into the simulator
            }
        ...
        base_rf = RandomForestClassifier(n_estimators=200, max_depth=8, min_samples_split=5, min_samples_leaf=3, class_weight='balanced', random_state=42, n_jobs=1)
        base_gb = GradientBoostingClassifier(n_estimators=150, max_depth=5, learning_rate=0.1, random_state=42)
        estimators = [('rf', base_rf), ('gb', base_gb)]
        
        if XGBOOST_AVAILABLE:
            freq = counts / counts.sum()
            spw = float(freq[0] / freq[1]) if len(freq) > 1 else 1.0
            estimators.append(('xgb', XGBClassifier(n_estimators=200, max_depth=6, learning_rate=0.08, subsample=0.8, colsample_bytree=0.8, scale_pos_weight=spw, random_state=42, use_label_encoder=False, eval_metric='mlogloss', verbosity=0)))

        meta_lr = LogisticRegression(max_iter=1000, C=1.0, class_weight='balanced', solver='lbfgs')
        skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)

        try:
            stacker = StackingClassifier(estimators=estimators, final_estimator=meta_lr, cv=skf, passthrough=False, n_jobs=1)
            ...
            fallback = Pipeline([('imputer', SimpleImputer(strategy='mean')), ('scaler', StandardScaler()), ('model', RandomForestClassifier(n_estimators=200, class_weight='balanced', random_state=42, n_jobs=1))])
            try:
...
class Disp:
    ...
    def header():
        print()
        print(C.cyan(" ╔══════════════════════════════════════════════════════════════╗"))
        print(C.cyan(" ║") + C.bold(" ⚽ PREMIER LEAGUE PREDICTOR PRO v4.0 ⚽ ") + C.cyan("║"))
        print(C.cyan(" ║") + C.dim(" Deep Stats CSV • XGBoost (68 Feats) • Classes Bugfix ") + C.cyan("║"))
        print(C.cyan(" ╚══════════════════════════════════════════════════════════════╝"))
        print()
    ...
    @staticmethod
    def backtest(r: dict):
        Disp.section("📊 BACKTEST v4.0")
        if 'error' in r:
            Disp.error(r['error'])
            return
        w = 62
        print(f" {C.blue('┌'+'─'*w+'┐')}")
        print(box(f" {C.bold('🔬 PERFORMANCE REPORT v4.0')}"))
        print(f" {C.blue('├'+'─'*w+'┤')}")
        ra = r['result_acc']
...
class App:
    ...
    def init(self) -> bool:
        ...
        self._log_msg('success', "Engine v4.0 ready! 🚀")
        return True
...
# ══════════════════════════════════════════════════════════════
def run_streamlit():
    st.set_page_config(page_title="PL Predictor v4.0", page_icon="⚽", layout="wide")
    st.markdown("<h1 style='text-align:center;color:#00ff9d;'>⚽ Premier League Predictor v4.0</h1>", unsafe_allow_html=True)
    st.caption("Deep Stats Integration • XGBoost (68 Feats) • Classes Bugfix • mg=10 Matrix")
    ...
    if 'initialized' not in st.session_state:
        st.info("👈 Enter API key and Initialize. Make sure E0 (1).csv is in the folder!")
        st.stop()
    ...
    if mode == "🔮 Predict Upcoming":
        ...
        if st.button("🔮 Predict"): st.session_state['custom_pr'] = app.custom(home, away)
        if st.session_state.get('custom_pr'):
            pr = st.session_state['custom_pr']
            st.write(f"### H: {pr.hp*100:.1f}% | D: {pr.dp*100:.1f}% | A: {pr.ap*100:.1f}%")
            st.write(f"🎯 Recommended DC: {pr.dc_recommend}")
        ...
        if r and 'error' not in r:
            ...
            c2.metric("⚽ Score Acc", f"{r['score_acc']:.1f}%")
            c3.metric("📊 Brier", f"{r['brier']:.4f}")
